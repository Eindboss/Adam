// app.core.patched.js — state, storage, utils, logic (patched: local fmtISO, richer filters, light migration)
(function(global){
  'use strict';
  ...
  // Patched: use local date parts to avoid UTC off-by-one
  ...
  function migrate(obj){
    // v1 -> v2: ensure weekStart is local date string (not UTC truncated) and add version
    if(!obj.__version){
      // If weekStart contains a 'Z', it's probably from toISOString().slice(0,10) without timezone issue; still ensure parse/format
      if (obj.weekStart) {
        try{
          const d=parseISO(obj.weekStart);
          obj.weekStart = fmtISO(d);
        }catch(_){}
      }
      obj.__version = 3;
    }
  ...
  function load(){
    try{
      ...
      if(!raw) return;
      const obj=migrate(JSON.parse(raw));
      ...
      state.view = (obj.view==='week') ? 'week' : 'day';
  ...
  // Patched: broader filter (title, notes, type, dueDate)
  ...
  function buildWeeklyStats(){
    ...
    const per={}; SUBJECTS.forEach(s=> per[s]={done:0,total:0, minutes:0});
    for(const t of state.tasks){
      if(dates.includes(t.plannedDate) && per[t.subject]){
        per[t.subject].total += 1;
        if(t.done) per[t.subject].done += 1;
        const est = Number(t.estimate||0);
        if(!t.done && est>0) per[t.subject].minutes += est;
      }
    }
    return per;
  }
  ...
  function copyTask(id){ const t=state.tasks.find(x=>x.id===id); if(!t) return; state.tasks.push({...t,id:uid(),title:(t.title||'Taak')+' (kopie)'}); }
  ...
  function planSplitTasks(base){
    ...
    return days.map((iso,i)=>({
      id:uid(), title:`${title} (${i+1}/${N})`, subject, type, estimate:splitMinutes,
      plannedDate:iso, dueDate:fmtISO(end), notes, done:false
    }));
  }
