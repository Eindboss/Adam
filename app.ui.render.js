...
(function(){
  ...
  function renderStudentDay(){
    ...
    const listEl = UI.el.studentTaskList;
    const titleEl = UI.el.studentDayTitle;
    const emptyEl = UI.el.studentEmpty;
    if(!listEl || !titleEl) return;

    titleEl.textContent = `${C.ucfirst(date.toLocaleDateString('nl-NL',{weekday:'long'}))} ${String(date.getDate()).padStart(2,'0')} ${date.toLocaleDateString('nl-NL',{month:'short'})}`;
    ...
    todays.forEach(t=>{
      ...
      check.onchange=()=>{ C.updateTask(t.id,{done:check.checked}); C.save(); renderStudentDay(); };
      ...
      const meta=H.ce('div','meta'); meta.textContent = `${t.type} · ${t.estimate} min` + (t.dueDate? ` · dl ${C.fmtShort(C.parseISO(t.dueDate))}` : '');
      mid.append(ti,meta);

      row.append(left,mid); card.appendChild(row); listEl.appendChild(card);
    });
  }
  ...
  function renderAll(){
    ...
    renderStats();
    if(C.state.uiMode==='student'){ renderStudentDay(); }
